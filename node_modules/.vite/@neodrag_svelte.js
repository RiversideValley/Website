// node_modules/.pnpm/@neodrag+svelte@1.1.3/node_modules/@neodrag/svelte/dist/index.js
...
var a = (t2, e2 = {}) => {
  ...
  p(T, $, t2, r2);
  let z, X, Y, j, k = "", O = !!f2;
  const P = document.body.style, U = t2.classList, W = (e3) => {
    const n3 = { offsetX: A, offsetY: R, domRect: t2.getBoundingClientRect() };
    t2.dispatchEvent(new CustomEvent("neodrag", { detail: n3 })), e3 == null ? void 0 : e3(n3);
  };
  ...
  function G(e3) {
    ...
    if (Y.contains(e3.target) && !(j == null ? void 0 : j.contains(e3.target)) && (N = true), !N)
      return;
    i2 && (k = P.userSelect, P.userSelect = "none"), W(E);
    const { clientX: r3, clientY: s2 } = c(e3) ? e3.touches[0] : e3, u2 = J();
    M && (B = r3 - T / u2), D && (H = s2 - $ / u2), z && (L = r3 - X.left, q = s2 - X.top);
  }
  function I() {
    N && (U.remove(b), U.add(w), i2 && (P.userSelect = k), W(C), M && (B = A), M && (H = R), N = false);
  }
  function K(e3) {
    if (!N)
      return;
    ...
    M && (A = (i3 - B) * l2), D && (R = (a3 - H) * l2), T = A, $ = R, W(x), S.then(() => p(A, R, t2, r2));
  }
  return { destroy: () => {
